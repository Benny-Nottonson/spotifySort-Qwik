...
export default async function sortManager(token: string, playlistId: string): Promise<boolean> {
    const tracks = await getPlaylist(token, playlistId);
    const tracksWithCCV = await Promise.all(tracks.map(async (track: {id: string, imageUrl: string}) => {
